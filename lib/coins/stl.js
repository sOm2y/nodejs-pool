...
function Coin(data){
    ...
    this.coinDevAddress = "Se2qr4ovTU2SiAbE1KyMLR4kQYKFWhB3V8mxijFfwdYTJVnw4ZdsA                                                                                        Hw294cHoypTYyc1sbUv2hTqr7q8GVBmSB9Q1a2L26BTn";  // Arq Address
    this.poolDevAddress = "Se2qr4ovTU2SiAbE1KyMLR4kQYKFWhB3V8mxijFfwdYTJVnw4ZdsA                                                                                        Hw294cHoypTYyc1sbUv2hTqr7q8GVBmSB9Q1a2L26BTn";  // Arq Address
...
//    this.intPrefix = 28822;
    ...
    this.getBlockHeaderByHash = function(blockHash, callback){
        global.support.rpcDaemon('getblockheaderbyhash', {"hash": blockHash}, fu                                                                                        nction (body) {
    ...
    this.baseDiff = function(){
        return bignum('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF                                                                                        FFFFFFF', 16);
    };
    ...
    this.BlockTemplate = function(template) {
        /*
        Generating a block template is a simple thing.  Ask for a boatload of in                                                                                        formation, and go from there.
        ...
        this.idHash = crypto.createHash('md5').update(template.blocktemplate_blo                                                                                        b).digest('hex');
        ...
        this.extraNonce = 0;
        // The clientNonceLocation is the location at which the client pools sho                                                                                        uld set the nonces for each of their clients.
        this.clientNonceLocation = this.reserveOffset + 12;
        // The clientPoolLocation is for multi-thread/multi-server pools to hand                                                                                        le the nonce for each of their tiers.
        this.clientPoolLocation = this.reserveOffset + 8;
        this.nextBlob = function () {
            // Write a 32 bit integer, big-endian style to the 0 byte of the res                                                                                        erve offset.
  ...
  this.cryptoNight = function(convertedBlob) {
            return multiHashing.cryptonight(convertedBlob, convertedBlob[0] >= 3                                                                                         ? 1:0);
        }
